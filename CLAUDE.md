...
Claude Code 세션을 자동 기록하고, 프롬프팅 습관을 회고하는 도구 모음.
...
SKILL.md                # /weekly-review 스킬 정의 (주간 습관 변화 추적)
SKILL-deep-dive.md      # /prompt-deep-dive 스킬 정의 (특정 세션 턴별 분석)
SKILL-export-review.md  # /export-review 스킬 정의 (리뷰 결과를 Notion/로컬 저장)
LOG-SPEC.md             # 로그 형식 스펙 (스킬 파일에서 include로 참조됨)
install.sh              # ~/.claude에 hook, 스킬, settings 설치
...
- **설치 구조**: install.sh가 이 디렉토리의 파일을 `~/.claude/hooks/`, `~/.claude/skills/` 등에 복사한다. 파일 추가/삭제 시 install.sh, uninstall.sh 모두 업데이트 필요.
- **스킬 간 관계**: weekly-review와 prompt-deep-dive는 같은 로그 데이터를 읽지만 분석 단위가 다름 (주간 vs 세션). export-review는 다른 스킬의 리포트 출력을 저장하는 후속 스킬.
