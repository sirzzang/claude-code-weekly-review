...
| 평균 세션 소요 시간 | elapsed_seconds 평균 | 맥락에 따라 다름 |
...
평균 세션 소요 시간    8m 20s   6m 10s    -2m 10s
```
...
- 지표가 악화되었더라도 작업 성격 변화(탐색적 작업이 많았던 주 등)가 원인일 수 있으므로, 숫자만으로 판단하지 않는다.
